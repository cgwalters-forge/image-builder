...
func (t *bootcImageType) genPartitionTable(customizations *blueprint.Customizations, rootfsMinSize uint64, rng *rand.Rand) (*disk.PartitionTable, error) {
	...
	if bd.unifiedKernel {
		...
		// A UKI has no root= karg, so the initrd finds the root filesystem
		// with systemd-gpt-auto-generator, by its partition type. A
		// partition table that comes from the container is used as-is.
		if bd.sourceInfo == nil || bd.sourceInfo.PartitionTable == nil {
